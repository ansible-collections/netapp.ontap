...
class NetAppONTAPSecurityAudit(object):
  ...
  def get_securiy_audit(self):
    ...
    if record is not None:
      for element in ('ontapi','cli','http'):
        value = record.get(f"{element}get")
  ...
  def patch_security_audit(self, m):

    attributes = self.security_audit_attr()
    message, error = rest_generic.patch_async(self.rest_api, self.api,None, m)
    if error:
      self.module.fail_json(msg=error)

  def security_audit_attr(self):
    body = {}
    for option in ('cli', 'http', 'ontapi'):
